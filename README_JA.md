...
現在は明示的な project alias とマルチプロジェクト session に対応しています。`bridge` や `SageTalk` のようなローカル repo を同じ WeChat bridge にぶら下げ、それぞれ独立した Codex session、history、mode、model を保持できます。
...
特定プロジェクトに 1 件だけ送る場合は、次のように送れます：

```text
@SageTalk run tests and summarize failures
```

このメッセージは `SageTalk` にだけルーティングされ、現在の active project は切り替わりません。
...
`~/.wechat-agent-bridge/config.json` で project alias を明示的に設定できます：
...
- `/project` で設定済みプロジェクト一覧と現在の active project を確認できます。
- `/project SageTalk` で active project を `SageTalk` に切り替えます。
- `@SageTalk テスト失敗の原因を見て` はその 1 件だけを `SageTalk` に送ります。
- `/interrupt SageTalk` は `SageTalk` の現在タスクを中断します。
- `/replace SageTalk この新しい方針でやり直して` は `SageTalk` の現在タスクを中断して置き換えます。

各プロジェクトは Codex session、history、mode、model を独立して保持します。別プロジェクト同士は同時実行できます。同じプロジェクトが処理中のときは、新しいメッセージは `/interrupt` か `/replace` を明示しない限り拒否されます。
